...
/*
 * This file is the main driver for the Secret Notes CLI example.
...
/* **********************************************************************
 * getNotesLedgerState: a helper that queries the current state of
 * the data on the ledger, for a specific notes contract.
...
You can do one of the following:
  1. Deploy a new notes contract
  2. Join an existing notes contract
  3. Exit
  ...
  while (true) {
    ...
    switch (choice) {
      case '1':
        ...
        return api;
      case '2':
        api = await NotesAPI.join(providers, await rli.question('What is the contract address (in hex)? '), logger);
 ...
 * by the contract to be in the ledger state of the notes contract.
 */

const displayLedgerState = async (
  providers: NotesProviders,
  deployedNotesContract: DeployedNotesContract,
  logger: Logger,
): Promise<void> => {
  const contractAddress = deployedNotesContract.deployTxData.public.contractAddress;
  ...
  if (ledgerState === null) {
    ...
    logger.info(`Current note commitments on-chain: ${ledgerState.notes.size()}`);
    logger.info(`Current nullifiers on-chain: ${ledgerState.nullifiers.size()}`);
  }
};
...
const displayPrivateState = async (providers: NotesProviders, logger: Logger): Promise<void> => {
  ...
  if (privateState === null) {
    logger.info(`There is no existing notes private state`);
  } else {
 ...
 * derived state is the list of notes that are locally known and still
 * valid (present as a commitment on-chain and not yet nullified).
 */

const displayDerivedState = (derivedState: NotesDerivedState | undefined, logger: Logger) => {
  if (derivedState === undefined || derivedState.notes.length === 0) {
    logger.info(`No notes currently available`);
  } else {
    for (const note of derivedState.notes) {
      logger.info(`- [${note.id.substring(0, 12)}...] '${note.title}'`);
    }
  }
};

/* **********************************************************************
 * mainLoop: the main interactive menu of the notes CLI.
...
const mainLoop = async (providers: NotesProviders, rli: Interface, logger: Logger): Promise<void> => {
  ...
  try {
    while (true) {
      ...
      try {
        switch (choice) {
          case '1': {
            const title = await rli.question(`Note title? `);
            const content = await rli.question(`Note content? `);
            await notesApi.createNote(title, content);
            break;
          }
          case '2': {
            const id = await rli.question(`Which note id do you want to update (hex)? `);
            const title = await rli.question(`New title? `);
            const content = await rli.question(`New content? `);
            await notesApi.updateNote(id, title, content);
            break;
          }
          case '3': {
            const id = await rli.question(`Which note id do you want to delete (hex)? `);
...
export const run = async (config: Config, testEnv: TestEnvironment, logger: Logger): Promise<void> => {
  ...
  try {
    ...
    const zkConfigProvider = new NodeZkConfigProvider<'createNote' | 'updateNote' | 'deleteNote' | 'readMyNotes'>(
      config.zkConfigPath,
    );
    const providers: NotesProviders = {
      privateStateProvider: levelPrivateStateProvider<PrivateStateId, NotesPrivateState>({
